...
def draw_game(current_state):
    ...
    vulnerable_sprite = pygame.image.load('sprites/fantasma1-2.png').convert_alpha()
    ...
    for ghost in ghosts_data:
        ...
        if ghost['state'] == 'vulnerable':
             current_ghost_image = vulnerable_sprite
        else:
...
def play():
    ...
    try:
        ...
        # 1. Handshake: Envia CONNECT
        connect_message = {'command': 'CONNECT'}
        client_socket.sendall(json.dumps(connect_message).encode('utf-8'))
        
        # 2. Handshake: Recebe Estado Inicial (Timeout de 1.0s)
        client_socket.settimeout(1.0)
        data = client_socket.recv(4096)
        
        if not data:
             # Isso garante que a exceção seja capturada, indicando falha no handshake
             raise ConnectionResetError("Servidor fechou a conexão durante o handshake.")
        ...
        while game_running:
            ...
            # 5. Recebe estado atualizado (Fast Timeout)
            try:
        ...
        print(f"Não foi possível conectar ao servidor em {HOST}:{PORT}. Verifique se está rodando.")
        
    except ConnectionResetError:
        print("Conexão perdida durante o handshake.")
